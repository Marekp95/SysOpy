    ...
    while (1) {
        ...
        if (result != NULL) {
            if (strlen(message_text) >= MAX_MESSAGE_LENGTH) {
                ...
                strcpy(to_send.message, message_text);
                wait_send = 1;
            }
        }
        pthread_cond_signal(&cond);
...
int main(int argc, char *argv[]) {
    ...
    if (global == 0) { // local
        ...
        port_number = (int) strtol(argv[4], NULL, 10);
    ...
    if (global == 0) {
        ...
        server_inet_address.sin_port = htons((uint16_t) port_number);
        server_address = (struct sockaddr *) &server_inet_address;
    }

    if (pthread_create(&pthread_t1, NULL, thread_fun, NULL) != 0) {
        perror(NULL);
        exit(-1);
    }
        ...
        if (select(socket1 + 1, &sockset1, NULL, NULL, NULL) > 0) {
            if (FD_ISSET(socket1, &sockset1)) {
                ...
                if (tmp == -1) {
                    ...
                    printf("Klient %s : %s\n", request1.sender, request1.message);
                } else {
                    printf("Rozłączony\n");
                    exit(1);
                }
